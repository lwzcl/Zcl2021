...
function StripUnprintable(const ASource: widestring; SubstChar: WideChar = '_'): widestring; overload;
function StripUnprintable(const ASource: ansistring; SubstChar: AnsiChar = '_'): ansistring; overload;
...
begin
  ...
  begin
    ...
    S := StringReplace(S, {$IFDEF RAD9PLUS}FormatSettings.{$ENDIF}ThousandSeparator, '', [rfReplaceAll]);
    // change DecimalSeperator to '.' because Val only recognizes that, not
    // the locale specific decimal char.  Stupid Val.
    S := StringReplace(S, {$IFDEF RAD9PLUS}FormatSettings.{$ENDIF}DecimalSeparator, '.', [rfReplaceAll]);
    // and try again
    Val(S, V, NumCode);
    Result := (NumCode = 0);
  End;
