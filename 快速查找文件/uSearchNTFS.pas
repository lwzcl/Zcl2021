...
procedure TSearchNTFS.Execute();
  ...
  begin
    ...
    begin
      ...
      while TArray.BinarySearch(ArrU64, ParentId, Item) do
      begin
...
, MAX_PATH   // 文件操作系统名称长度
    ...
    if SameText(Path, 'NTFS') then
    begin
      // 2.打开磁盘 (需要管理员权限)
      hTmp := 0;
      hFile := Winapi.Windows.CreateFile(PWideChar('\\.\' + ADiskName + ':'), Generic_Read + Generic_Write      // 打开卷 操作方式(读或写)
        , File_Share_Read + File_Share_Write// 共享方式
        , nil, Open_Existing                     // 文件创建方法
      ...
      // 3.初始化USN日志文件
      if not DeviceIoControl(hFile, FSCTL_CREATE_USN_JOURNAL, @CUSN, Sizeof(CUSN), nil, 0, dwRet, nil) then
      begin
        ErrStr := '初始化USN日志错误：' + SysErrorMessage(GetLastError());
        Exit;
      end;
      // 4.获取USN日志基本信息
      if not DeviceIoControl(hFile, FSCTL_QUERY_USN_JOURNAL, nil, 0, @USN, Sizeof(USN), dwRet, nil) then
      begin
      ...
      while DeviceIoControl(hFile, FSCTL_ENUM_USN_DATA, @MEnum, Sizeof(MEnum), @Buf, Len_Buf, dwRet, nil) do
      begin
        // 找到第一个 USN 记录
      ...
      // 8.释放内存
      for Idx := AFiles.Count - 1 downto 0 do
      begin
...
begin
  ...
  AFiles.BeginUpdate;
  if form2.CB_all.Checked = True then
  begin
    for AStr in TDirectory.GetLogicalDrives do
    begin
      Synchronize(
        procedure
        begin
          Form2.Lab_start.Caption := ' 正在扫描 ' + AStr;
        end);
      AFiles.Clear;
      SearchNTFS(AStr.Chars[0], AFiles, AErr);
      for Idx := 0 to AFiles.Count - 1 do
      begin
    ...
    Synchronize(
      procedure
      begin
        Form2.Lab_start.Caption := ' 扫描完成 ' + AErr;
      end);
